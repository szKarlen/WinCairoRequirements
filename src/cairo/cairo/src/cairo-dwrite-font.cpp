					    ...
					    cairo_scaled_font_t **font)
{
    ...
    *font = reinterpret_cast<cairo_scaled_font_t*>(dwriteFont);
	...
	//font_face->font->CreateFontFace(&font_face->dwriteface);
			    ...
			    uint32_t		      ucs4)
{
    ...
    UINT16 index;
    face->dwriteface->GetGlyphIndicesW(&ucs4, 1, &index);
    return index;
}
				 ...
				 int			*remaining_glyphs)
{
	...
	if (generic_surface->backend && generic_surface->backend->type == CAIRO_SURFACE_TYPE_WIN32 &&
	...
	status = (cairo_int_status_t)_cairo_surface_composite(op, generic_surface, &mask_surface->base, generic_surface, source_x, source_y, 0, 0, dest_x, dest_y, width, height, clip_region);
	
	_cairo_pattern_fini (&mask.base);

	cairo_pattern_destroy(&mask.base);
...
public:
	...
	*ppObject = static_cast<IDWriteGeometrySink*>(this);
	...
	_controlfp_s(&control_word, _CW_DEFAULT, _PC_64);
...
{
    cairo_image_surface_t *image24 = (cairo_image_surface_t *)((cairo_win32_display_surface_t*)mask_surface)->image;
...
cairo_dwrite_font_face_create_for_dwrite_fontface(void* dwrite_font, void* dwrite_font_face)
{
	...
	//face->font = dwritefont;
	//face->font->CreateFontFace(&face->dwriteface);
...
{
    cairo_dwrite_scaled_font_t *font = reinterpret_cast<cairo_dwrite_scaled_font_t*>(dwrite_scaled_font);
    if (force && font->rendering_mode == TEXT_RENDERING_NORMAL) {
        font->rendering_mode = TEXT_RENDERING_GDI_CLASSIC;
    } else if (!force && font->rendering_mode == TEXT_RENDERING_GDI_CLASSIC) {
        font->rendering_mode = TEXT_RENDERING_NORMAL;
    }
}
...
{
    cairo_dwrite_scaled_font_t *font = reinterpret_cast<cairo_dwrite_scaled_font_t*>(dwrite_scaled_font);
    return font->rendering_mode == TEXT_RENDERING_GDI_CLASSIC;
}
...
cairo_font_face_t*
cairo_logfontw_font_face_create_for_dwrite(cairo_scaled_font_t* scaled_font)
{
	...
	cairo_dwrite_font_face_t* dwrite_font_face = (cairo_dwrite_font_face_t*)scaled_font->font_face;
	...
	LOGFONTW logFont;
	BOOL isSystem;
	gdiInterop->ConvertFontToLOGFONT(dwrite_font_face->font, &logFont, &isSystem);

	return cairo_win32_font_face_create_for_logfontw(&logFont);
}
...
static DWRITE_FONT_WEIGHT LogFontWeightToDWrite(LONG lfWeight)
{
	switch (lfWeight)
	{
	case FW_DONTCARE: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_REGULAR;
	case FW_THIN: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_THIN;
	case FW_ULTRALIGHT: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_ULTRA_LIGHT;
	case FW_LIGHT: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_LIGHT;
	case FW_NORMAL: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_REGULAR;
	case FW_MEDIUM: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_MEDIUM;
	case FW_SEMIBOLD: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_SEMI_BOLD;
	case FW_BOLD: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_BOLD;
	case FW_EXTRABOLD: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_EXTRA_BOLD;
	case FW_HEAVY: return DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_HEAVY;
	default:
...
cairo_dwrite_font_face_create_for_logfontw_internal(LOGFONTW *logfont, IDWriteFontCollection* collection)
{
	...
	if (FAILED(gdiInterop->CreateFontFromLOGFONT(logfont, &dwriteFont))) {
		_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
	}
	...
	if (FAILED(dwriteFont->CreateFontFace(&dwriteFontFace))) {
		_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
	}

	//return cairo_dwrite_font_face_create_for_dwrite_fontface(dwriteFont, dwriteFontFace);

	cairo_dwrite_font_face_t *face = new cairo_dwrite_font_face_t;
	cairo_font_face_t *font_face;

	dwriteFontFace->AddRef();

	//face->font = dwritefont;
	//face->font->CreateFontFace(&face->dwriteface);
	face->dwriteface = dwriteFontFace;
	face->font = dwriteFont;
...
void* cairo_dwrite_font_face_get(cairo_scaled_font_t* scaled_font)
{
	...
	return dwrite_font_face->dwriteface;
}
...
void
_cairo_region_boxes_fini(cairo_region_t *region, cairo_box_t *boxes)
{
	free(boxes);
}
...
//		boxes = _cairo_region_get_boxes(region, &num_boxes);
//
//		if (num_boxes == 1 &&
//			boxes[0].p1.x == 0 &&
//			boxes[0].p1.y == 0 &&
					...
					cairo_clip_t* clip)
{
	...
	if (clip != NULL) {
		if ((dst->flags & CAIRO_WIN32_SURFACE_FOR_PRINTING) == 0) {
			if (!_cairo_win32_display_surface_set_clip((cairo_win32_display_surface_t*)dst, clip))
				return CAIRO_INT_STATUS_NO_MEMORY;
		}
	}
	else {
		_cairo_win32_display_surface_unset_clip((cairo_win32_display_surface_t*)dst);
	}
